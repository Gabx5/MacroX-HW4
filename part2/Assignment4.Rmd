...
For this exercise we tried to change $\lambda_1$ holding $\lambda_2$ constant, then varying $\lambda_2$ holding $\lambda_1$, and then varying both $\lambda_1$ and $\lambda_2$. For each of those, we compare 6 different values of lambda. The corresponding coefficients are depicted in the first 3 figures in Appendix. Also in the later figures in Appendix IRFs and forecast plots are depicted.
...
```{r figure 3 SP500, eval=FALSE, include=FALSE}
#New storage for cumulative IRFs
...
```
For the sign restrictions we extended the identification from above and used:
...
Where other means other sources of shocks to oil.

```{r SR SP500, eval=FALSE, include=FALSE}
# Code for replication sign restriction 
#New storage for cumulative IRFs
