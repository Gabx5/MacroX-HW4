...
```{r part2, echo=FALSE, include=FALSE}
#------------------------------------------------------------------------------------
# PRIORS
#------------------------------------------------------------------------------------
...
lambda1 <- 0.0001; lambda2 <- 0.0005; lambda3 <- 100
...
Plots should start here:

```{r plot 1, echo=FALSE, eval=FALSE}
plot.ts(Yraw)
```
...
```{r plot 4, echo=FALSE, fig.width=12, fig.height=7}
result_table <- data.frame(Round = numeric(), Mean = numeric(), Quantile_5 = numeric(), Quantile_95 = numeric())
...
for(ii in 1:k){
  for(jj in 1:M){
    ...
    Z_scores[(jj-1)*k+ii] <- geweke.diag(A_store[,ii,jj])$z
     ...
     mean_val <- mean(A_store[, ii, jj])
    sd_val <- sd(A_store[, ii, jj])
    quantile_5 <- quantile(A_store[, ii, jj], p = 0.05)
    quantile_95 <- quantile(A_store[, ii, jj], p = 0.95)
...
num_rows <- length(result_table) / 3
# Convert the column vector into a matrix with 3 columns
mean_table <- matrix(result_table[,1], ncol = 3, byrow=TRUE)
sd_table <- matrix(result_table[,2], ncol = 3, byrow=TRUE)
```
